    ...
    def all(self):
        ...
        def list_genres(tx):
            cypher_statement="""
                        ...
                        ORDER BY g.name ASC
                    """
            res = tx.run(cypher_statement)
            return [ g.value(0) for g in res]

        with self.driver.session() as session:
            return session.read_transaction(list_genres)
        # return genres
    # end::all[]
    ...
    def find(self, name):
        ...
        def get_genre(tx, name):
            cypher_statement="""
                    ...
                    """
            first =  tx.run(cypher_statement, name=name).single()
        ...
        with self.driver.session() as session:
            return session.read_transaction(get_genre,name)
