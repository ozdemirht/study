 ...
 * Every alphabetic character is replaced with the character 3 letters
 * higher (wrapping around from Z to A), and every numeric character
...
public class Main {
    ...
    char charRotated(char c, char base, int mod, int rotationFactor) {
        int diff = (c - base);
        diff += rotationFactor;
        diff = diff % mod;
    ...
    String rotationalCipher(String input, int rotationFactor) {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            if (Character.isDigit(output[i])) {
                output[i] = charRotated(output[i], '0', 10, rotationFactor);
            } else if (Character.isLowerCase(output[i])) {
                output[i] = charRotated(output[i], 'a', 26, rotationFactor);
            } else if (Character.isUpperCase(output[i])) {
                output[i] = charRotated(output[i], 'A', 26, rotationFactor);
            }
        }
